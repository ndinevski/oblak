...
func containerWith(name, status string, ports ...[2]int) pristanisteContainer {
	c := pristanisteContainer{Name: name, Status: status}
	for _, p := range ports {
...
func TestDesiredRoutesSinglePort(t *testing.T) {
	got := desiredRoutes([]pristanisteContainer{
	...
	if r.Source != models.SourcePristaniste {
		t.Errorf("source = %q, want pristaniste", r.Source)
	}
	if r.Target != "webapp" {
...
func TestDesiredRoutesSkipsUnpublishedAndUDP(t *testing.T) {
	c := pristanisteContainer{Name: "svc", Status: "running"}
	...
	got := desiredRoutes([]pristanisteContainer{c}, "localhost")
	if len(got) != 0 {
