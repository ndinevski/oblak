...
Pristaniste supplies the management API and the dashboard integration. The storage and
...
scope. Pristaniste is a single-node self-hosted service; a "service" with one replica
...
```bash
# Push to Pristaniste's registry
...
Note the image reference is the bare `my-app:v1`. Pristaniste resolves an unqualified
...
| Variable | Purpose |
|---|---|
| `PRISTANISTE_API_PORT` | API port (8083) |
| `PRISTANISTE_REGISTRY_PORT` | Published registry port (5000) |
| `REGISTRY_URL` | Where Pristaniste reaches the registry, over the compose network |
| `PRISTANISTE_REGISTRY_PUBLIC_HOST` | What users put in an image reference |
| `PRISTANISTE_REGISTRY_USERNAME` / `_PASSWORD` | Registry credentials, if it requires them |
| `DOCKER_GID` | Host docker group id, needed to read `docker.sock` |

`REGISTRY_URL` and `PRISTANISTE_REGISTRY_PUBLIC_HOST` differ on purpose: Pristaniste talks to
...
**Pristaniste needs the host's container engine, and `docker.sock` is root-equivalent.**
That is inherent to running containers, not something Pristaniste adds. Two things
limit the blast radius:

- Every container Pristaniste creates is labelled `io.oblak.pristaniste.managed`, and Pristaniste
...
Neither changes the fact that the socket is mounted. **Do not expose Pristaniste's API
...
trusted network. Set `PRISTANISTE_REGISTRY_USERNAME` and `PRISTANISTE_REGISTRY_PASSWORD`, and
...
Pristaniste reports traces, RED metrics and trace-correlated logs to the Oblak
collector like every other service. The Docker SDK also instruments its own
transport, so engine calls appear as client spans inside a Pristaniste trace, which
