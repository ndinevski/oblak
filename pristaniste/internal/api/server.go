...
import (
	...
	"github.com/oblak/pristaniste/internal/telemetry"
)

// Server is the Pristaniste API server.
type Server struct {
	router   *mux.Router
...
type Config struct {
	...
	// RegistryURL is where Pristaniste reaches the registry API.
	RegistryURL string
	// RegistryPublicHost is what users put in an image reference. It differs
	// from RegistryURL whenever Pristaniste reaches the registry over a container
...
func GetConfigFromEnv() Config {
	return Config{
		Port:               envOr("PRISTANISTE_PORT", "8083"),
		DockerHost:         os.Getenv("DOCKER_HOST"),
		RegistryURL:        envOr("REGISTRY_URL", "http://pristaniste-registry:5000"),
...
//
// Pristaniste is degraded rather than down when only one backend is unreachable: with
