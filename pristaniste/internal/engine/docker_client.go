...
// ManagedLabel marks every container Pristaniste creates.
//
// Pristaniste shares the host's Docker daemon with whatever else is running on it,
// including the rest of the Oblak platform. Listing and mutating only labelled
// containers means Pristaniste can never stop Postgres or delete the telemetry
// collector because someone clicked the wrong row.
const ManagedLabel = "io.oblak.pristaniste.managed"

// OwnerLabel records which Oblak user created the container.
const OwnerLabel = "io.oblak.pristaniste.owner"
...
type DockerConfig struct {
	...
	// Pristaniste's own registry can be run by short name.
	RegistryHost string
}
...
func NewDockerClient(cfg DockerConfig) (*DockerClient, error) {
	opts := []client.Opt{
		// Negotiation keeps Pristaniste working against older and newer daemons
...
// managedFilter restricts an operation to containers Pristaniste created.
...
func (d *DockerClient) GetContainer(ctx context.Context, idOrName string) (*models.Container, error) {
	...
	// Refuse to expose containers Pristaniste does not manage, so the API cannot be
...
func (d *DockerClient) CreateContainer(ctx context.Context, req *models.CreateContainerRequest) (*models.Container, error) {
	...
	for k, v := range req.Labels {
		// Pristaniste's own labels are applied last so a caller cannot spoof them.
		if k == ManagedLabel {
			continue
		}
		labels[k] = v
...
// qualifyImage prefixes a bare repository name with Pristaniste's registry, so
// "my-app:v1" resolves to an image pushed to Pristaniste rather than Docker Hub.
//
