 ...
 * Containers.
 *
 * Running workloads launched from images in Pristaniste's registry, with lifecycle
...
export default function ContainersPage() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<PristanisteContainer | null>(null);
  const [logsFor, setLogsFor] = useState<PristanisteContainer | null>(null);
  ...
  const runAction = async (c: PristanisteContainer, verb: 'start' | 'stop' | 'restart') => {
    try {
      ...
      {health.data && health.data.status === 'degraded' && (
        <Card className="border-amber-500/20 bg-amber-500/5">
          <CardContent className="flex items-center gap-2 py-3 text-sm">
            ...
            <span>
              Pristaniste is degraded: engine {health.data.engine}, registry {health.data.registry}.
            </span>
          </CardContent>
        </Card>
      )}
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            An unqualified image name resolves against Pristaniste's own registry.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              list="pristaniste-image-suggestions"
            />
            <datalist id="pristaniste-image-suggestions">
              {suggestions.map((s) => (
...
function LogsDialog({ container, onClose }: { container: PristanisteContainer; onClose: () => void }) {
