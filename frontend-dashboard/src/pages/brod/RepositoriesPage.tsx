 ...
 * Image repositories.
 *
 * The registry side of Brod: what has been pushed, how big it is, and how to
 * push more.
 */
...
export default function RepositoriesPage() {
  ...
  const registry = useBrodRegistry();
  const repositories = useRepositories();
  const removeRepository = useDeleteRepository();

  const [expanded, setExpanded] = useState<string | null>(null);
  const [creating, setCreating] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState<BrodRepository | null>(null);
...
function ImageRow({ image }: { image: BrodImage }) {
  const { toast } = useToast();
