/**
 * Tests for the Pristaniste API client and its presentation helpers.
 *
...
describe('pristanisteApi request paths', () => {
  it('unwraps the Strapi data envelope', async () => {
    mockGet.mockResolvedValue({ data: { data: [{ name: 'my-app' }] } } as never);

    const repos = await pristanisteApi.listRepositories();
    expect(repos).toEqual([{ name: 'my-app' }]);
  });

  it('escapes a simple repository name', async () => {
    await pristanisteApi.getRepository('my-app');
    expect(mockGet.mock.calls[0][0]).toBe('/pristaniste/repositories/my-app');
  });
  ...
  it('keeps slashes in a namespaced repository name as path separators', async () => {
    ...
    await pristanisteApi.getRepository('team/my-app');
    expect(mockGet.mock.calls[0][0]).toBe('/pristaniste/repositories/team/my-app');
  });

  it('escapes characters within a repository component', async () => {
    await pristanisteApi.getRepository('team/my app');
    expect(mockGet.mock.calls[0][0]).toBe('/pristaniste/repositories/team/my%20app');
  });

  it('escapes the tag when deleting an image', async () => {
    await pristanisteApi.deleteImage('team/my-app', 'v1.0+build');
    expect(mockDelete.mock.calls[0][0]).toBe(
      '/pristaniste/repositories/team/my-app/images/v1.0%2Bbuild'
    );
  });

  it('lists containers including stopped ones by default', async () => {
    await pristanisteApi.listContainers();
    expect(mockGet.mock.calls[0][0]).toBe('/pristaniste/containers?all=true');
  });
  ...
  it('forces removal when deleting a container', async () => {
    ...
    await pristanisteApi.deleteContainer('web');
    expect(mockDelete.mock.calls[0][0]).toBe('/pristaniste/containers/web?force=true');
  });

  it('passes the tail through to the logs endpoint', async () => {
    await pristanisteApi.containerLogs('web', 50);
    expect(mockGet.mock.calls[0][0]).toBe('/pristaniste/containers/web/logs?tail=50');
  });
});
